...
class ViewController: UIViewController, helperDelegate{
    ...
    var showBonus:UILabel!
    var showTotal:UILabel!
    ...
    func gameOver(timer: Timer) -> Void {
        ...
        gameOverImage.image = UIImage(named: "gameO.jpeg")
...
//        // customise the view
        showScore = UILabel(frame: CGRect(x:width/6, y:height/2, width:250, height:51))
        showBonus = UILabel(frame: CGRect(x:width/6, y:height/2 + 20, width:250, height:51))
        showTotal = UILabel(frame: CGRect(x:width/6, y:height/2 + 60, width:250, height:51))
        
            showScore.text = " Your  score: " + scoreText.text!
            showBonus.text = " Your  bonus: " +  bonus.text!
            //var total = Int(scoreText.text!) + Int(bonus.text!)
            
            let a:Int? = Int(scoreText.text!)// firstText is UITextField
            let b:Int? = Int(bonus.text!)// secondText is UITextField
            let total = a! + b!
            showTotal.text = " Total score:  " + String(total)
            //showScore.font = showScore.font.withSize(40)
            //showScore.backgroundColor = .orange

        showScore.font = UIFont(name: "Verdana-Bold", size: 12.0)
        showScore.textColor = UIColor.white
        showScore.font = showScore.font.withSize(20)
        
        showBonus.font = UIFont(name: "Verdana-Bold", size: 12.0)
        showBonus.textColor = UIColor.white
        showBonus.font = showScore.font.withSize(20)

        showTotal.textColor = UIColor.white
        showTotal.font = showScore.font.withSize(23)
        
        // show on screen
        self.view.addSubview(showScore)
        self.view.addSubview(showBonus)
        self.view.addSubview(showTotal)
    ...
    @objc func buttonAction(sender: UIButton!) {
        ...
        showBonus.removeFromSuperview()
        showTotal.removeFromSuperview()
        ...
        for view in self.view.subviews{
            if((view.tag == 50 || view.tag == 80) && player.frame.intersects(view.frame)){
                if(view.tag == 80){view.removeFromSuperview()
                    ...
                    a = a! + 15
                    bonus.text =  String(a!)
                    player.image = UIImage(named:"staremoji.png")
                }else{
                //score = score - (i/1000)
                i = i+1
                //print(score)
    ...
    func drawPlayButton(){
        ...
        playButton = UIButton(frame: CGRect(x: UIScreen.main.bounds.size.width - 130, y: UIScreen.main.bounds.size.height - 90, width: UIScreen.main.bounds.size.width/1.7, height: 50))
        playButton.backgroundColor = .orange
        playButton.layer.cornerRadius = 0.1 * playButton.bounds.size.width
